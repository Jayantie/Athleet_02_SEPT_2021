...
public class AtleetManager {
	...
	// Jayantie oplossing	
			for (SportAtleet atletenobject : atletenList) {
				if (atletenobject.getSport().equals(searchSportType)) {
					String voornaam = atletenobject.getVoornaam();
					if (firstName.equals("") || firstName.equals(voornaam)) {	
						atletenobject.showAtleet();
					}
				}
			}
	}
	...
	//		Jayantie oplossing	
			for (SportAtleet atleten : atletenList) {
				boolean sportIndividueel = atleten.getSport().isIndividueel();
				if (zoekIndividueel == sportIndividueel) {
					System.out.println("Name: "+atleten.getVoornaam()
							+" "+atleten.getAchternaam()+ 
							" Sport: " + atleten.getSport().toString());
				} 
			}		
	}
...
//			Jayantie oplossing for loops			
			marchBornPlayers += (atleet.getGeboortedatum().getMonth().equals(Month.MARCH)) ? 1:0;
		}
